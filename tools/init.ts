...
const renameFiles = [
  ["src/library.ts", "src/safe-hook.ts"],
  ["test/library.test.ts", "test/safe-hook.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/safe-hook/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/safe-hook/g, libraryName)
    mv(
